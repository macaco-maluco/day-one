...
export default ({
  ...
  const isMain =
    selectedSolarSystem && selectedSolarSystem.stage === SOLAR_SYSTEM_STAGES.MAIN_SEQUENCE
      ...
      {selectedSolarSystem &&
        ...
        >
          ...
          >
              ...
              {isMain && selectedSolarSystem.dysonSwarm != null
                ? selectedSolarSystem.dysonSwarm.currentEnergy > 0
                    ? <div>
                        ...
                        >
                          {selectedSolarSystem.dysonSwarm.currentEnergy < 2000
                            ? 'Collecting...'
                            : 'Harvest'}
                        </button>
                      </div>
            ...
            {isMain &&
              !selectedSolarSystem.dysonSwarm &&
              selectedSolarSystem.planets.map((p, i) => (
                <div key={i} style={{ paddingRight: '30px', width: 160 }}>
                  <h3>Planet {i}</h3>
                  ...
                  {p.material === originalMaterial &&
                    <div>
                      ...
                      <button onClick={() => onClickOnboard(i)} disabled={!canOnboard(p)}>
                        Onboard
                      </button>
                    </div>}
                </div>
...
const canOnboard = planet => planet.currentPopulation >= POPULATION_ONBOARD_SIZE
